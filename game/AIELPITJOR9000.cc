 ...
 */
#define PLAYER_NAME ELPITJOR9000
...
struct PLAYER_NAME : public Player {
  ...
  void explorar_fuel(Pos pos_expl, queue<Pos>& cua, Pos anterior, MI& dist, MB& explorat, MP& cami) {
    if (not only_road(pos_expl, true)) return;
    int i=pos_expl.i;
    int j=pos_expl.j;
    /*bool kamikaze=false;
    if ( cell(pos_expl).id!=-1 and not warrior(pos_expl) ) kamikaze=true;*/
  ...
  int bfs_fuel(Pos inici, Pos& desti, MI& dist, MB& explorat, MP& cami){
    queue<Pos> cua;
    cua.push(inici);
    while (not cua.empty()){
      Pos act=cua.front();
      ...
      for (int k=0; k<8; ++k){
          explorar_fuel(act+Dir(k), cua, act, dist, explorat, cami);
      }
    }
    return -1;
  }
  ...
  void explorar_c(Pos pos_expl, queue<Pos>& cua, Pos anterior, MI& dist, MB& explorat, MP& cami){
    if (not only_road(pos_expl, false)) return;
    int i=pos_expl.i;
    int j=pos_expl.j;
    /*bool kamikaze=false;
    if ( cell(pos_expl).id!=-1 and not warrior(pos_expl) ) kamikaze=true;*/
  ...
  int buscar_desti_c(Pos inici, Pos& desti, MI& dist, MB& explorat, MP& cami){
    queue<Pos> cua;
    cua.push(inici);
    while (not cua.empty()){
      Pos act=cua.front();
      ...
      for (int k=0; k<8; ++k){
          explorar_c(act+Dir(k), cua, act, dist, explorat, cami);
      }
    }
    return -1;
  }
      ...
      for (int y=0; y<n; ++y) {
          ...
          if (can_move(id)) {
            ...
            if (not ha_atacat) {
                ...
                bool fuel=false;
                MI dist_fuel( 60, vector<int>(60, -1) );
                MB explorat_fuel( 60, vector<bool>(60, false) );
                MP cami_fuel( 60, vector<Pos>(60) );
                dist_fuel[inici.i][inici.j]=0;
                explorat_fuel[inici.i][inici.j]=true;
                int fueh=bfs_fuel(inici, desti_fuel, dist_fuel, explorat_fuel, cami_fuel);
                if (fueh+8>=gazo) fuel=true;
                if (fuel) {
                    int i=desti_fuel.i;
                    int j=desti_fuel.j;
                    while(dist_fuel[desti_fuel.i][desti_fuel.j]>=2) desti_fuel=cami_fuel[desti_fuel.i][desti_fuel.j];
                    int d;
                    on_anar(inici, desti_fuel, d);
                    ...
                    int k=buscar_desti_c(inici, desti, dist, explorat, cami); //cap a warrior
